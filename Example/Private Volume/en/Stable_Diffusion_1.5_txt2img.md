...
After your code is debugged, execute the `everai app prepare` command. This command will execute all methods annotated by `@app.prepare`. At this time, the value of `is_prepare_mode` is `True`. In the sample code, the model files of volume `models--runwayml--stable-diffusion-v1-5` in the local environment will be pushed to the cloud when this command is executed.   
